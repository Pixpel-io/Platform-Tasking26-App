...
export function MailClient() {
  ...
  return <div className="relative flex h-full min-h-0 flex-col bg-background">
    ...
    <div className="grid min-h-0 flex-1 md:grid-cols-[96px_340px_minmax(0,1fr)] xl:grid-cols-[104px_390px_minmax(0,1fr)]">
      <nav aria-label="Mailbox actions" className="relative hidden min-h-0 flex-col items-center overflow-hidden border-r border-border/60 bg-linear-to-b from-surface via-surface/85 to-surface-2/45 px-2.5 py-4 shadow-[8px_0_28px_rgba(0,0,0,0.035)] backdrop-blur-2xl md:flex">
        ...
        <p className="relative mb-3 text-[9px] font-bold uppercase tracking-[0.22em] text-muted/75">Mail tools</p>
        <button onClick={() => setCompose(true)} className="group relative flex w-full flex-col items-center gap-2 overflow-hidden rounded-2xl border border-primary/30 bg-linear-to-br from-primary via-primary to-primary/75 px-2 py-3.5 text-primary-foreground shadow-[0_10px_30px_rgba(var(--primary-rgb),0.22)] transition duration-200 hover:-translate-y-0.5 hover:border-white/20 hover:shadow-xl hover:shadow-primary/25 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-primary/25"><span aria-hidden="true" className="absolute inset-x-2 top-0 h-px bg-linear-to-r from-transparent via-white/60 to-transparent" /><span className="grid h-8 w-8 place-items-center rounded-xl bg-white/12 ring-1 ring-white/15 transition-transform duration-200 group-hover:rotate-[-4deg] group-hover:scale-105"><Icon d="M12 20h9M16.5 3.5a2.12 2.12 0 0 1 3 3L8 18l-4 1 1-4z" className="h-4.5 w-4.5" /></span><span className="text-[11px] font-bold tracking-tight">Compose</span></button>
        <div className="my-4 h-px w-12 bg-linear-to-r from-transparent via-border to-transparent" />
...
function MailRailButton({ label, children, onClick, active = false, danger = false, disabled = false, badge = 0 }: { label: string; children: ReactNode; onClick: () => void; active?: boolean; danger?: boolean; disabled?: boolean; badge?: number }) {
  return <button type="button" onClick={onClick} disabled={disabled} aria-label={label} title={label} className={`group relative flex w-full flex-col items-center gap-1.5 rounded-2xl border px-1 py-2.5 text-[10px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-3 disabled:cursor-not-allowed disabled:opacity-45 ${danger ? "border-transparent text-muted hover:border-danger/20 hover:bg-danger/8 hover:text-danger focus-visible:ring-danger/20" : active ? "border-primary/20 bg-primary/10 text-primary shadow-sm shadow-primary/8" : "border-transparent text-muted hover:-translate-y-px hover:border-border/70 hover:bg-background/75 hover:text-foreground hover:shadow-sm focus-visible:ring-primary/20"}`}><span className={`relative grid h-8 w-8 place-items-center rounded-xl border transition-all duration-200 ${danger ? "border-border/60 bg-background/60 group-hover:border-danger/20 group-hover:bg-danger/8" : active ? "border-primary/25 bg-primary/12 shadow-inner" : "border-border/60 bg-background/65 shadow-sm group-hover:border-primary/20 group-hover:bg-primary/8 group-hover:text-primary"}`}>{children}{badge > 0 && <span className="absolute -right-2.5 -top-2 grid h-4.5 min-w-4.5 place-items-center rounded-full border-2 border-surface bg-primary px-1 text-[9px] font-bold leading-none text-primary-foreground shadow-sm shadow-primary/30">{badge > 99 ? "99+" : badge}</span>}</span><span className="max-w-full truncate tracking-tight">{label}</span>{active && <span aria-hidden="true" className="absolute bottom-1 h-0.5 w-4 rounded-full bg-primary" />}</button>;
}
